...
- **Worker on Render's free Web Service tier** spins down after ~15 minutes of inactivity, causing a delayed response (30–60s) on the first job after being idle. An external uptime-ping service (e.g. cron-job.org) can be pointed at the worker's health-check URL to mitigate this, but it's not a substitute for an always-on paid instance.
